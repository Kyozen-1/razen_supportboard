...
@section('title', 'Razen Supportboard | Produk Razen')
...
@section('content')
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title">Razen Supportboard | Produk Razen</h1>
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    <li class="breadcrumb-item"><a href="{{ route('razen-supportboard.admin.dashboard.index') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="#">Produk Razen</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="produk_razen_table" class="data-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Nama</th>
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#produk_razen_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-supportboard.admin.produk-razen.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#testimoni_form').on('submit', function(e){
            ...
            {
                $.ajax({
                    url: "{{ route('razen-supportboard.admin.produk-razen.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#testimoni_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#produk_razen_table').DataTable().ajax.reload();
                        }
            ...
            {
                $.ajax({
                    url: "{{ route('razen-supportboard.admin.produk-razen.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-supportboard.admin.produk-razen.show', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_gambar').attr('src', "{{ asset('images/razen-supportboard/produk-razen') }}" + '/' + data.result.gambar);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-supportboard.admin.produk-razen.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
                success: function(data)
                {
                    ...
                    var lokasi_gambar = "{{ asset('images/razen-supportboard/produk-razen') }}"+'/'+data.result.gambar;
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-supportboard.admin.produk-razen.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            $.ajax({
                url: url,
                beforeSend: function(){
