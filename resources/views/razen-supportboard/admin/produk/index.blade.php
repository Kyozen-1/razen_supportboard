...
@section('title', 'Razen Supportboard | Produk')
...
@section('content')
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title">Razen Supportboard | Produk</h1>
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="produk_table" class="data-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Judul</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="produk_form" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="mb-3 position-relative form-group">
                            ...
                            <input type="text" class="form-control" name="ikon" id="ikon" required>
                        </div>
                        <div class="mb-3 position-relative form-group">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group mb-3 position-relative">
                        ...
                        <input type="text" id="detail_ikon" class="form-control" disabled>
                    </div>
                    <div class="form-group mb-3 position-relative">
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#produk_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-supportboard.admin.produk.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#produk_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('razen-supportboard.admin.produk.store') }}",
                    method: "POST",
            ...
            {
                $.ajax({
                    url: "{{ route('razen-supportboard.admin.produk.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-supportboard.admin.produk.show', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_ikon').val(data.result.ikon);
                    $('#detail_gambar').attr('src', "{{ asset('images/razen-supportboard/produk') }}" + '/' + data.result.gambar);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-supportboard.admin.produk.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
                success: function(data)
                {
                    ...
                    var lokasi_gambar = "{{ asset('images/razen-supportboard/produk') }}"+'/'+data.result.gambar;
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-supportboard.admin.produk.destroy', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            $.ajax({
                url: url,
                beforeSend: function(){
